...
A tally lamp: the amber bar the masthead has drawn since the first screen. A
tally is the lamp on a broadcast camera that lights when it is the one going
out — the most characteristic object in this app's subject, and where the
interface's single accent colour comes from.

The wordmark is drawn as geometry rather than set in a typeface, so the logo
depends on no font licence, and because the letterforms wanted here are
monoline with square terminals — the lettering of instrument panels rather
than of a text face.

All of it is generated by
[`apps/opentv/tool/generate_assets.py`](apps/opentv/tool/generate_assets.py),
so the artwork can be re-derived and a palette change is one edit rather than
twenty exports.
