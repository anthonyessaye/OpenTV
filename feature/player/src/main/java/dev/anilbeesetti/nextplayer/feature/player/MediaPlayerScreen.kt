...
fun MediaPlayerScreen(
    ...
    fun seekRelative(ms: Long) {
        ...
        player.seekTo(pos.fastCoerceIn(0, player.duration))
    }
                    ...
                    .onKeyEvent { keyEvent ->
                        if (keyEvent.type == KeyEventType.KeyDown) {
                            when {
                                ...
                                //keyEvent.key == Key.DirectionUp    -> { con }
                                //keyEvent.key == Key.DirectionDown  -> { adjustVolume(-1);    true }
                                keyEvent.key == Key.DirectionCenter -> {
                                    controlsVisibilityState.unlockControls(); true
                                }
                                ...
                                keyEvent.key == Key.MediaPlay -> {
                                    player.play(); true
                                }

                                keyEvent.key == Key.MediaPause -> {
                                    player.pause(); true
                                }

                                keyEvent.key == Key.MediaStop -> { /* optional */      true
                                }
                                //keyEvent.key == Key.MediaNext      -> { viewModel.next();   true }
                                //keyEvent.key == Key.MediaPrevious  -> { viewModel.prev();   true }

                                // ── Back / escape ──
                                keyEvent.key == Key.Back || keyEvent.key == Key.Escape -> {
                                    if (controlsVisibilityState.controlsVisible && player.isPlaying == true) {
                                        controlsVisibilityState.hideControls()
                                        true
                                    }
                                ...
                                keyEvent.key == Key.S && keyEvent.isCtrlPressed -> {
                                    onSelectSubtitleClick(); true
                                }

                                else -> false   // let event propagate
                            }
                        }
